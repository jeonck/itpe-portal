...
**다음 중 하나 이상을 선택하여 작성합니다. 여러 개 사용 가능하지만 토픽의 성격에 맞게 선택하세요.**
...
**언제 사용:** 수행해야 할 단계별 절차나 프로세스를 설명할 때

**동작원리와의 차이:**
- **동작원리**: 시스템/메커니즘이 어떻게 작동하는지 설명 (예: DevOps CI/CD 파이프라인 동작 과정)
- **절차**: 사람이 수행해야 할 단계별 프로세스 설명 (예: 감리 수행 절차, 프로젝트 관리 절차)

```markdown
## 절차
업무 수행 절차를 단계별로 설명합니다.
...
### 3. 기술요소/동작원리/기능/절차 선택 가이드

| 토픽 유형 | 추천 섹션 | 예시 |
|---------|----------|------|
| 인프라/아키텍처 | 기술요소 | API Gateway, Kubernetes, Service Mesh |
| 자동화 프로세스 | 동작원리 | Agile, DevOps, CI/CD 파이프라인 |
| 수동 프로세스/관리 | 절차 | 소프트웨어 감리, 프로젝트 관리, 요구공학 |
| 시스템/플랫폼 | 기능 | MSA, ERP, CRM, Low-Code |
| 알고리즘 | 동작원리 | 정렬, 탐색, 그래프 알고리즘 |
| 보안 기술 | 기술요소 또는 동작원리 | 암호화, 인증, 방화벽 |

**동작원리 vs 절차 구분:**
- **동작원리**: 시스템/메커니즘의 자동 작동 과정 (예: CI/CD 파이프라인이 어떻게 동작하는가)
- **절차**: 사람이 수행하는 단계별 업무 프로세스 (예: 감리를 어떻게 수행하는가)

**복수 사용 가능:**
- 큰 토픽의 경우 여러 섹션 조합 가능
- 예: Kubernetes (기술요소: 컴포넌트, 동작원리: 스케줄링 과정)
- 예: 프로젝트 관리 (절차: 5대 프로세스, 기술요소: 관리 도구)
