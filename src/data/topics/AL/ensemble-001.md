...
# 정의
여러 약한 학습기를 결합하여 강력한 예측 모델을 만드는 기법 기법.
...
## 동작원리
- Bagging 동작: 1) Bootstrap Sampling으로 여러 학습 데이터셋 생성 (중복 허용 무작위 추출) → 2) 각 데이터셋으로 독립적인 모델 학습 → 3) 예측 시 모든 모델의 결과를 평균(회귀) 또는 다수결 투표(분류)로 결합
